...
var CGameLogic = cc.Class({
    ...
    GetCardType:function ( cbCardData, cbCardCount)
    {
        ...
        if (AnalyseResult.cbBlockCount[3]>0)
        {
            ...
            if ((AnalyseResult.cbBlockCount[3]==1) && (cbCardCount==6)) return GameDef.CT_FOUR_TAKE_ONE;
            if ((AnalyseResult.cbBlockCount[3]==1)&&(cbCardCount==8)&&(AnalyseResult.cbBlockCount[1]==2) ||
            (AnalyseResult.cbBlockCount[3]==2 )&&(cbCardCount==8)) return GameDef.CT_FOUR_TAKE_TWO;
        ...
        if (AnalyseResult.cbBlockCount[2]>0)
        {
            ...
            else if( cbCardCount == 3 ) return GameDef.CT_THREE;

            //牌形判断
            if( AnalyseResult.cbBlockCount[2]==2 )
            {
                if(AnalyseResult.cbBlockCount[0]>0&&AnalyseResult.cbBlockCount[1]==0){
                    if(AnalyseResult.cbBlockCount[0]==2 )
                    {
                        return GameDef.CT_AIRPLANE_ONE;
                    }
                }else if(AnalyseResult.cbBlockCount[1]>0&&AnalyseResult.cbBlockCount[0]==0){
                    if(AnalyseResult.cbBlockCount[1]==2 )
                    {
                        return GameDef.CT_AIRPLANE_TWO;
                    }
                }
            }

            if (AnalyseResult.cbBlockCount[2]*3==cbCardCount) return GameDef.CT_THREE_LINE;
            if (AnalyseResult.cbBlockCount[2]*4==cbCardCount) return GameDef.CT_THREE_TAKE_ONE;
            if ((AnalyseResult.cbBlockCount[2]*5==cbCardCount)&&(AnalyseResult.cbBlockCount[1]==AnalyseResult.cbBlockCount[2])) return GameDef.CT_THREE_TAKE_TWO;
    ...
    SortOutCardList:function ( cbCardData, cbCardCount)
    {
        ...
        if( cbCardType == GameDef.CT_THREE_TAKE_ONE || cbCardType == GameDef.CT_THREE_TAKE_TWO )
        {
            //分析牌
            var AnalyseResult =new tagAnalyseResult();
            this.AnalysebCardData( cbCardData,cbCardCount,AnalyseResult );
        ...
        //飞机
        else if(cbCardType == GameDef.CT_AIRPLANE_ONE || cbCardType == GameDef.CT_AIRPLANE_TWO)
        {
            //分析牌
            var AnalyseResult =new tagAnalyseResult();
            this.AnalysebCardData( cbCardData,cbCardCount,AnalyseResult );
    ...
    //出牌搜索
    SearchOutCard:function ( cbHandCardData, cbHandCardCount, cbTurnCardData, cbTurnCardCount, pSearchCardResult )
    {
        //设置结果
        ...
        case GameDef.CT_ERROR:					//错误类型
            {
                ...
                //三带一单
                cbTmpCount = this.SearchTakeCardType( cbCardData,cbCardCount,0,3,1,tmpSearchCardResult );
                if( cbTmpCount > 0 )
                {
                    pSearchCardResult.cbCardCount[cbResultCount] = tmpSearchCardResult.cbCardCount[0];
                    for(var i=0;i<tmpSearchCardResult.cbCardCount[0];i++)
                ...
                //三带一对
                cbTmpCount = this.SearchTakeCardType( cbCardData,cbCardCount,0,3,2,tmpSearchCardResult );
                if( cbTmpCount > 0 )
                {
                    pSearchCardResult.cbCardCount[cbResultCount] = tmpSearchCardResult.cbCardCount[0];
                    for(var i=0;i<tmpSearchCardResult.cbCardCount[0];i++)
                ...
                //飞机
                var cbTmpCount = this.SearchThreeTwoLine( cbCardData,cbCardCount,0, 0, 0, tmpSearchCardResult );
                if( cbTmpCount > 0 )
                {
                ...
                // if( cbSameCount != 4 )
                // {
                //     cbTmpCount = this.SearchSameCard( cbCardData,cbCardCount,0,4,tmpSearchCardResult );
                //     if( cbTmpCount > 0 )
                //     {
                //         pSearchCardResult.cbCardCount[cbResultCount] = tmpSearchCardResult.cbCardCount[0];
                //         for(var i=0;i<tmpSearchCardResult.cbCardCount[0];i++)
        ...
        case GameDef.CT_THREE_TAKE_TWO:	//三带一对
            {
                //效验牌数
                ...
                if( cbTurnCardCount == 4 || cbTurnCardCount == 5 )
                {
                    ...
                    //搜索三带牌型
                    cbResultCount = this.SearchTakeCardType( cbCardData,cbCardCount,cbTurnCardData[2],3,cbTakeCardCount,pSearchCardResult );
                }
                else
                {
                    //变量定义
        ...
        case GameDef.CT_FOUR_TAKE_TWO:		//四带两双
            {
                var cbTakeCount = cbTurnOutType==GameDef.CT_FOUR_TAKE_ONE?1:2;
                ...
                //搜索带牌
                cbResultCount = this.SearchTakeCardType( cbCardData,cbCardCount,cbTmpTurnCard[0],4,cbTakeCount,pSearchCardResult );

                break;
            }
        case GameDef.CT_AIRPLANE_ONE://飞机带单
        case GameDef.CT_AIRPLANE_TWO://飞机带对
            {
                var cbTakeCount = cbTurnOutType==GameDef.CT_AIRPLANE_ONE?1:2;

                var cbLineCount = cbTakeCount == 1 ? cbTurnCardCount/4 : cbTurnCardCount/5;
                ...
                this.SortOutCardList( cbTmpTurnCard,cbTurnCardCount );
                //搜索边牌
                cbResultCount = this.SearchThreeTwoLine( cbCardData,cbCardCount,cbTmpTurnCard[0], cbLineCount, cbTakeCount, pSearchCardResult );
                break;
            }
        }
    ...
    //带牌类型搜索(三带一，四带一等)
    SearchTakeCardType:function ( cbHandCardData, cbHandCardCount, cbReferCard, cbSameCount, cbTakeCardCount, pSearchCardResult )
    {
        //设置结果
        if( pSearchCardResult )
        {
    ...
    //搜索飞机
    SearchThreeTwoLine:function ( cbHandCardData, cbHandCardCount, cbReferCard, cbLineCount, cbTakeCount, pSearchCardResult )
    {
        //设置结果
        if( pSearchCardResult )
        {
        ...
        //搜索连牌
        cbTmpResultCount = this.SearchLineCardType( cbHandCardData,cbHandCardCount,cbReferCard,3,cbLineCount,tmpSearchResult );
        ...
        if( cbTmpResultCount > 0 )
        {
            ...
            for( var i = 0; i < cbTmpResultCount; i++ )
            {
                ...
                if(cbTakeCount == 0 || cbTakeCount == 1)
                {
                    var index = tmpSingleWing.cbSearchCount;
                    tmpSingleWing.cbCardCount[index] = (tmpSearchResult.cbCardCount[i] / 3) * 4;
                    ...
                    if(TmpResult.cbBlockCount[0] >= tmpSearchResult.cbCardCount[i]/3)
                    {
                        for (var j = 0; j < tmpSearchResult.cbCardCount[i]/3; j++)
                        {
                           tmpSingleWing.cbResultCard[index][tmpSearchResult.cbCardCount[i]+j] = TmpResult.cbCardData[0][j];
                        }
                    }
                    else
                    {
                        var nWingCount = 0;
                        //先把单牌带走 再拆对拆三拆四
                        for(var j = 0; j < TmpResult.cbBlockCount.length; j++)
                        {
                            for(var k = 0; k < TmpResult.cbBlockCount[j]; k++)
                            {
                                tmpSingleWing.cbResultCard[index][tmpSearchResult.cbCardCount[i]+nWingCount] = TmpResult.cbCardData[j][k + k * (j+1)];
                                nWingCount++;
                                if(nWingCount >= tmpSearchResult.cbCardCount[i]/3)
                                {
                                    break;
                                }
                            }
                        ...
                        if(nWingCount < tmpSearchResult.cbCardCount[i]/3) return 0;
                    }

                    tmpSingleWing.cbSearchCount++;
                }
                if(cbTakeCount == 0 || cbTakeCount == 2)
                {

                    var index = tmpDoubleWing.cbSearchCount;
                    tmpDoubleWing.cbCardCount[index] = (tmpSearchResult.cbCardCount[i] / 3) * 5;
                    for (var j = 0; j < tmpSearchResult.cbCardCount[i]; j++)
                    {
                        tmpDoubleWing.cbResultCard[index][j] = tmpSearchResult.cbResultCard[i][j];
                    }
                    //先把对带走 再拆对拆三拆四
                    if(TmpResult.cbBlockCount[1] >= tmpSearchResult.cbCardCount[i]/3)
                    {
                        for (var j = 0; j < (tmpSearchResult.cbCardCount[i]/3)*2; j++)
                        {
                            tmpDoubleWing.cbResultCard[index][tmpSearchResult.cbCardCount[i]+j] = TmpResult.cbCardData[1][j];
                        }
                    }
                    else
                    {
                        ...
                        for(var j = 1; j < TmpResult.cbBlockCount.length; j++)
                        {
                            for(var k = 0; k < TmpResult.cbBlockCount[j]; k++)
                            {
                                tmpDoubleWing.cbResultCard[index][tmpSearchResult.cbCardCount[i]+nWingCount] = TmpResult.cbCardData[j][k + k * (j+1)];
                                nWingCount++;
                                tmpDoubleWing.cbResultCard[index][tmpSearchResult.cbCardCount[i]+nWingCount] = TmpResult.cbCardData[j][k+1 + k * (j+1)];
                                nWingCount++;

                                if(nWingCount >= (tmpSearchResult.cbCardCount[i]/3)*2)
                                {
                                    break;
                                }
                        ...
                        if(nWingCount < (tmpSearchResult.cbCardCount[i]/3)*2) return 0;
                    }
