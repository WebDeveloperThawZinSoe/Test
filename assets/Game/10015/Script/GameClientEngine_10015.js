...
cc.Class({
    ...
    OnEventGameMessage: function (wSubCmdID, pData, wDataSize) {
        switch (wSubCmdID) {
            ...
            case GameDef.SUB_S_ASK_BANKER:
            case GameDef.SUB_S_ASK_CHANGE_BANKER:
                {
                    return this.OnSubAskBanker(pData, wDataSize);
                }
                case GameDef.SUB_S_ASK_RESULT:
                    {
                        return this.OnAskResultData(pData, wDataSize);
                    }
            ...
            case GameDef.GS_TK_FREE:	//空闲状态
                {
                    ...
                    //保存数据
                    this.m_wBankerUser = pStatusFree.wBankerUser;
                    if(this.m_wBankerUser != GameDef.INVALID_CHAIR)
                    {
                        var BankViewID = this.SwitchViewChairID(pStatusFree.wBankerUser);
                        this.m_GameClientView.m_AniBanker = pStatusFree.wBankerUser;
                
                        this.m_GameClientView.SetBankerUser(BankViewID, false);
                    }
                    //更新坐庄人数
                    this.m_GameClientView.UpdateBankerProgress(pStatusFree.wBankedUserCnt);
                    //庄家分数
                    this.m_GameClientView.SetBankerScore(pStatusFree.llBankerScore);
                    //桌面庄家信息
                    this.m_GameClientView.SetBankerTableInfo(pStatusFree.cbBankLevel,pStatusFree.cbBankCnt);
                    //更新玩家积分
                    this.m_GameClientView.OnUpdateUserScore(pStatusFree.llPlayerScore);
                    return true;
                }
            ...
            case GameDef.GS_TK_GAME_END:
                {
                    ...
                    if(this.m_wBankerUser != GameDef.INVALID_CHAIR)
                    {
                        var BankViewID = this.SwitchViewChairID(pStatusPlay.wBankerUser);
                        this.m_GameClientView.m_AniBanker = pStatusPlay.wBankerUser;
                
                        this.m_GameClientView.SetBankerUser(BankViewID, false);
                    }
                    //更新坐庄人数
                    this.m_GameClientView.UpdateBankerProgress(pStatusPlay.wBankedUserCnt);
                    //庄家分数
                    this.m_GameClientView.SetBankerScore(pStatusPlay.llBankerScore);
                    //桌面庄家信息
                    this.m_GameClientView.SetBankerTableInfo(pStatusPlay.cbBankLevel,pStatusPlay.cbBankCnt);
                    //更新玩家积分
                    this.m_GameClientView.OnUpdateUserScore(pStatusPlay.llPlayerScore);
                    ...
                    for (var i = 0; i < GameDef.GAME_PLAYER; i++) {
                        ...
                        if (pStatusPlay.llBankerTimes[i] != 0xFF && 
                            (cbGameStatus == GameDef.GS_TK_CALL_BANKER || cbGameStatus == GameDef.GS_TK_BANKER_ANIM))
                        ...
                        //玩家手牌
                        this.m_GameClientView.m_UserCardControl[ViewID].SetCardData(pStatusPlay.cbHandCardData[i], pStatusPlay.iSendCardCount, pStatusPlay.cbOxType[i],GameDef.MAX_COUNT - 2);
                        ...
                        if (ViewID != GameDef.MYSELF_VIEW_ID)
                            this.m_GameClientView.ShowCuoPaiState(ViewID, pStatusPlay.cbPlayStatus[i] == this.PLAYER_STATUS_CUO_PAI);
                    }
                    return true;
                }
            case GameDef.GS_TK_ASK_BANKER:
                {
                    //效验数据
                    var pStatusPlay = GameDef.CMD_S_StatusAsk();
                    if (wDataSize != gCByte.Bytes2Str(pStatusPlay, pData)) return false;
                    ...
                    var pIClientUserItem = this.GetClientUserItem(pStatusPlay.wBankerUser);
                    var _nickName = pIClientUserItem.GetNickName();
                    //提示
                    switch (pStatusPlay.cbAskMode)
                    {
                        case 0:
                            this.m_GameClientView.SetTableTips('等待' + _nickName + '是否下庄：');
                            break;
                        case 1:
                            this.m_GameClientView.SetTableTips('等待' + _nickName + '是否下庄：');
                            break;
                        case 2:
                            this.m_GameClientView.SetTableTips('等待' + _nickName + '是否上庄：');
                            break;
                    }
            
                    if (MeChair == pStatusPlay.wBankerUser)
                        this.m_GameClientView.ShowAskUI(bGame && !this.IsLookonMode(),pStatusPlay.cbAskMode);
            
                    //设置时间
                    this.SetGameClock(GameDef.MYSELF_VIEW_ID, IDI_GAME_CLOCK, pStatusPlay.dwCountDown / 1000);
    ...
    OnSubStartCall: function (pData, wDataSize) {
        ...
        this.m_GameClientView.SetTableTips('等待玩家抢庄：');
    ...
    OnSubCallBanker: function (pData, wDataSize) {
        ...
        this.m_GameClientView.SetUserCallBankerTimes(ViewID, UserCall.cbTimes, true);

        if (UserCall.cbTimes != 0xFF)
            this.PlayActionSound(UserCall.wCurrentUser, 'CB_' + UserCall.cbTimes)
    ...
    OnSubBankerUser: function (pData, wDataSize) {
        ...
        this.m_GameClientView.m_AniUserArr = null;

        this.m_GameClientView.SetBankerUser(BankViewID, !this.m_ReplayMode);

        //更新坐庄人数
        this.m_GameClientView.UpdateBankerProgress(BankerUser.wBankedUserCnt);
        //庄家分数
        this.m_GameClientView.SetBankerScore(BankerUser.llBankerScore);
        //桌面庄家信息
        this.m_GameClientView.SetBankerTableInfo(BankerUser.cbBankLevel, BankerUser.cbBankCnt);
        //更新玩家积分
        this.m_GameClientView.OnUpdateUserScore(BankerUser.llPlayerScore);
    ...
    OnSubStartJetton: function (pData, wDataSize) {
        ...
        var MeChair = this.GetMeChairID();
        var bGame = this.m_UserState[MeChair];
        this.m_GameClientView.SetTableTips('等待玩家下注：');
    ...
    OnSubChangeJetInfo: function (pData, wDataSize) {
        ...
        this.m_GameClientView.SetPlayerUIBtnInfo(ChangeInfo.llBetScore,ChangeInfo.llMinBetScore,ChangeInfo.llMaxBetScore);
    ...
    OnSubCallPlayer: function (pData, wDataSize) {
        ...
        var ViewID = this.SwitchViewChairID(UserCall.wCurrentUser);
        this.m_GameClientView.SetUserBet(ViewID, UserCall.llScore, true);

        if (UserCall.cbIsShoot == 1)
            this.m_GameClientView.ShowShootAni(ViewID);
        return true;
    },
    ...
    OnSubHintCard: function (pData, wDataSize) {
        ...
        this.m_GameClientView.m_UserCardControl[ViewID].SetCardData(OpenCard.cbCardData, GameDef.MAX_COUNT, OpenCard.cbOxType,GameDef.MAX_COUNT - 2);
    ...
    OnSubShowStartCtrl: function (pData, wDataSize) {
        ...
        if (CtrlStart.bIsNeedReady)
            this.m_GameClientView.SetTableTips('等待玩家准备：');
        else if (CtrlStart.bIsNeedStart)
            this.m_GameClientView.SetTableTips('等待首位开始：');
        else
            this.m_GameClientView.SetTableTips('等待游戏开始：');
    ...
    OnSubOpenResult: function (pData, wDataSize) {
        ...
        for (var i = 0; i < GameDef.GAME_PLAYER; i++) {
            ...
            //玩家手牌
            this.m_GameClientView.m_UserCardControl[ViewID].SetCardData(pResult.cbCardData[i], GameDef.MAX_COUNT, pResult.cbOxType[i],GameDef.MAX_COUNT - 2);
    ...
    //庄家询问
    OnSubAskBanker: function (pData, wDataSize){
        var pAsk = GameDef.CMD_S_AskBanker();
        //效验
        if (wDataSize != gCByte.Bytes2Str(pAsk, pData)) return false;
        ...
        var pIClientUserItem = this.GetClientUserItem(pAsk.wBankerUser);
        var _nickName = pIClientUserItem.GetNickName();
        //提示
        switch (pAsk.cbAskMode)
        {
            case 0:
                this.m_GameClientView.SetTableTips('等待' + _nickName + '是否下庄：');
                break;
        ...
        if (MeChair == pAsk.wBankerUser)
            this.m_GameClientView.ShowAskUI(bGame && !this.IsLookonMode(),pAsk.cbAskMode);

        //设置时间
        this.SetGameClock(GameDef.MYSELF_VIEW_ID, IDI_GAME_CLOCK, pAsk.dwCountDown / 1000);

        return true;
    },

    //桌面信息
    OnAskResultData: function (pData, wDataSize) {
        var UpdateData = GameDef.CMD_S_AskResult();
        //效验
        if (wDataSize != gCByte.Bytes2Str(UpdateData, pData)) return false;

        //保存数据
        this.m_wBankerUser = UpdateData.wBankerUser;
        if(this.m_wBankerUser != GameDef.INVALID_CHAIR)
        {
            var BankViewID = this.SwitchViewChairID(UpdateData.wBankerUser);
            this.m_GameClientView.m_AniBanker = UpdateData.wBankerUser;
    
            this.m_GameClientView.SetBankerUser(BankViewID, false);
        }

        //更新坐庄人数
        this.m_GameClientView.UpdateBankerProgress(UpdateData.wBankedUserCnt);
        //庄家分数
        this.m_GameClientView.SetBankerScore(UpdateData.llBankerScore);
        //桌面庄家信息
        this.m_GameClientView.SetBankerTableInfo(UpdateData.cbBankLevel, UpdateData.cbBankCnt);
        //更新玩家积分
        this.m_GameClientView.OnUpdateUserScore(UpdateData.llUserScore);
    ...
    OnSubGameConclude: function (pData, wDataSize) {
        ...
        if (this.m_GameEnd.bIsNormal)
        {
            ...
            for (var i = 0; i < GameDef.GAME_PLAYER; i++) {
                ...
                this.m_GameClientView.SetBankerEndScore(this.m_GameEnd.llBankerResult);
    
                this.m_GameClientView.m_UserCardControl[ViewID].SetCardData(this.m_GameEnd.cbCardData[i], GameDef.MAX_COUNT, this.m_GameEnd.cbOxType[i],GameDef.MAX_COUNT - 2);
            ...
            //庄家分数
            this.m_GameClientView.SetBankerScore(this.m_GameEnd.llBankerScore);
            //更新玩家积分
            this.m_GameClientView.OnUpdateUserScore(this.m_GameEnd.llUserScore);
        }
    ...
    OnTimeIDI_DEALAY_SCORE_ANIM: function () {
        ...
        this.m_GameClientView.SetEndScoreAnim(this.m_wBankerUser, this.m_GameEnd.wWinRandArr);
    },
    ...
    OnMessageCtrlStart: function () {
        ...
        // if (!this.nView)this.nView = 0;
        // this.m_GameClientView.ShowShootAni(this.SwitchViewChairID(this.nView));
        // this.nView++;
        // if (this.nView > this.m_GameClientView.m_playerCnt)this.nView = 0;
    ...
    OnMessageCallBanker: function (callScore) {
        ...
        //发送数据
        var CallScore = GameDef.CMD_C_CallBanker();
    ...
    OnMessageCallPlayer: function (callScore,iIndex) {
        ...
        CallScore.cbIsShoot = parseInt(iIndex);
    ...
    //庄操作
    OnMessageBankerOperate: function (cbOperateMode) {
        var pData = GameDef.CMD_C_BankerOperate();
        pData.cbOperateMode = cbOperateMode;
        this.SendGameData(GameDef.SUB_C_BankerOpearte, pData);
    },
    ...
    SetViewRoomInfo: function (dwServerRules, dwRulesArr) {
        ...
        this.m_bIsHoldBank = dwRulesArr[0] & GameDef.GAME_TYPE_GAME_4;
