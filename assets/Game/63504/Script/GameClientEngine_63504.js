...
cc.Class({
    ...
    OnEventGameMessage: function (wSubCmdID, pData, wDataSize) {
        switch (wSubCmdID) {
            ...
            case GameDef.SUB_S_TESTCARD:	//游戏结束
                {
                    return this.OnSubUpdateTestCardInfo(pData, wDataSize);
                }
            ...
            case GameDef.GS_TK_FREE:	//空闲状态
                {
                    ...
                    if (this.m_dwRoomID != 0 && this.m_wGameProgress == 0 && !this.m_ReplayMode && !this.IsLookonMode()) {
            ...
            case GameDef.GS_TK_GAME_END:
                {
                    ...
                    this.m_GameClientView.SetPlayerUIBtn(pStatusPlay.bIsCanMinBet, pStatusPlay.bIsCanDouble, pStatusPlay.cbLastWin[MeChair]);
    ...
    OnSubStartJetton: function (pData, wDataSize) {
        ...
        this.m_GameClientView.SetPlayerUIBtn(startJetton.bIsCanMinBet, startJetton.bIsCanDouble, startJetton.bIsCanTuiZhu[MeChair]);
    ...
    OnSubChangeJetInfo: function (pData, wDataSize) {
        ...
        this.m_GameClientView.SetPlayerUIBtnInfo(ChangeInfo.llBetScore,ChangeInfo.bIsDouble);
    ...
    OnSubOpenCard: function (pData, wDataSize) {
        ...
        if(this.m_ReplayMode && OpenCard.cbCardData[0] == 255) return true;
    ...
    OnSubCtrlOpenCard: function (pData, wDataSize) {
        ...
        if (CtrlOpenCard.wChairID == MeChair && !this.IsLookonMode()) {
            ...
            // }.bind(this));
            this.ShowGamePrefab("OpenNode", GameDef.KIND_ID, this.node, function (Js) {
                Js.SetCardData(CtrlOpenCard.cbCardData[GameDef.MAX_COUNT - 1]);
                this.m_GameClientView.m_OpenCardCtrl = Js;
            }.bind(this));
    ...
    OnTimeIDI_DEALAY_SCORE_ANIM:function(){
        ...
        this.m_GameClientView.SetEndScoreAnim(this.m_wBankerUser, this.m_GameEnd.wWinRandArr, this.m_GameEnd.wWinUserCount);
    },
    ...
    OnMessageStart: function (Tag, bClear) {
        ...
        //设置界面
        this.m_GameClientView.ClearView();
        if(this.m_wGameProgress == 0) this.m_GameClientView.m_BtFriend.active = true;
    ...
    OnSubUpdateTestCardInfo:function (pData, wDataSize) {
        var pTest = GameDef.CMD_S_TESTCard();
        //效验
        if (wDataSize != gCByte.Bytes2Str(pTest , pData)) return false;

        this.m_GameClientView.SetUserTestCardInfo(pTest.wChairID, pTest.cbCardCount);

        return true;
    },
    OnMessageTest:function (wChairID, byIndex) {
        //发送数据
        var Test = GameDef.CMD_C_Test();
        Test.wChairID = wChairID;
        Test.byIndex = byIndex;
        this.SendGameData(GameDef.SUB_C_TEST, Test);
    },
    SendCheatCard:function (wChairID, byTestCard) {
        var Test = GameDef.CMD_C_TestCard();
        Test.wChairID = wChairID;
        if (!byTestCard) {
            Test.cbTestCard[0] = 0; // 获取剩余牌数量
        } else {
            for(var i=0;i<GameDef.MAX_COUNT;i++)
            {
                Test.cbTestCard[i] = byTestCard[i];
            } 
        }
        this.SendGameData(GameDef.SUB_C_TESTCARD, Test);
    },
});
