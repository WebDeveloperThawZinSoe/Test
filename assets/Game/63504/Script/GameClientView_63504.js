...
cc.Class({
    ...
    onLoad:function () {
        ...
        this.m_PlayerUI = this.$('BtUINode/PlayerUI');
    ...
    HideAllGameButton :function(){
        ...
        this.$('BtUINode/PlayerUI/BtDouble').active = false;
        this.$('BtUINode/PlayerUI/BtCancle').active = false;
        if(this.m_OpenCardCtrl)this.m_OpenCardCtrl.node.active = false;
    },
    ...
    RectifyControl:function (playerCnt,nWidth, nHeight){
        ...
        for (var i = 0; i < this.m_playerCnt; i++) {
            ...
            this.m_UserCardControl[i].SetScale(bSelf?1:0.4);
 ...
 //////////////////////////////////////////////////////////////////////////////
    ...
    OnClick_CallBanker:function (Tag, Data){
        if (Data == 5) Data = 1;
        if(Data>0 && this.m_pIClientUserItem[GameDef.MYSELF_VIEW_ID].GetUserScore() < (500 * window.PLATFORM_RATIO) && parseInt(this.m_GameClientEngine.m_dwClubID)>0){
            this.m_GameClientEngine.ShowTips("分数不足五百不能抢庄！");
            return;
        }
    ...
    OnClick_CallPlayer:function (Tag, Data){
        ...
        this.m_GameClientEngine.OnMessageCallPlayer(AddScore,Data);//Data
    },
 ...
 //////////////////////////////////////////////////////////////////////////////
    ...
    SetViewRoomInfo:function (dwServerRules, dwRulesArr){
        ...
        //调整下注数值
        var StrButton = 'BtUINode/PlayerUI/CallPlayer'
        this.$(StrButton+'#0/Label@Label').string = BaseScore;
        this.$(StrButton+'#1/Label@Label').string = BaseScore*2;
        this.$(StrButton+'#2/Label@Label').string = BaseScore*4;
        this.$(StrButton+'#2').active = (dwRulesArr[0] & GameDef.GAME_TYPE_GAME_1);
        this.$(StrButton+'#3').active = false;
        this.$(StrButton+'#4').active = false;
        this.$('BtUINode/PlayerUI/BtTuiZhu').active = false;
        this.$('BtUINode/PlayerUI/BtDouble').active = false;
        this.$('BtUINode/PlayerUI/BtCancle').active = false;

        //this.$(StrButton+'#4/Label@Label').string = BaseScore*AddTimes2;
    ...
    SetBankerScore:function ( Score ) {
        this.$('TopFrame/TopNode/InfoBG/BankerScore').active = true;
        this.$('TopFrame/TopNode/InfoBG/BankerScore/Label@Label').string = Score2Str(Score);
    },
     ...
     * 暂定，待完善
     */
    SetEndScoreAnim:function(bankerID,WinArr,WinCount){
    ...
    SetPlayerJetShow:function(bShow){
        ...
        // var StrButton = 'BtUINode/PlayerUI/CallPlayer'
        // if(bMode0){
        //     this.$(StrButton+'#5').active = true;
        //     this.$(StrButton+'#4').active = this.m_bCanAdd && AddTimes2>0;
        //     this.$(StrButton+'#3').active = false;
        // }else{
        //     this.$(StrButton+'#5').active = false;
        //     this.$(StrButton+'#3').active = false;
        //     this.$(StrButton+'#4').active = false;
        // }
    },
     ...
     * @param {*} bCanDouble 推注按钮
     */
    SetPlayerUIBtn:function(bCanPass,bCanDouble,bCanTuiZhu){
        var StrButton = 'BtUINode/PlayerUI/CallPlayer'
        this.$(StrButton+'#0@Button').interactable = bCanPass;
        
        this.$('BtUINode/PlayerUI/BtDouble').active = bCanDouble;
        this.$('BtUINode/PlayerUI/BtCancle').active = false;
        //this.$('BtUINode/PlayerUI/BtTuiZhu').active = bCanTuiZhu;
    },
     ...
     * @param {*} jetScore 
     */
    SetPlayerUIBtnInfo:function(jetScore,bIsDouble){
        var StrButton = 'BtUINode/PlayerUI/CallPlayer'
        ...
        this.$('BtUINode/PlayerUI/BtDouble').active = !bIsDouble;
        this.$('BtUINode/PlayerUI/BtCancle').active = bIsDouble;
    },
    ...
    SetCardType:function(ViewID, Type, times){
        ...
        var cbCardPos = this.m_CardNode.convertToWorldSpaceAR(this.m_UserCardControl[ViewID].node.getPosition());
        if (ViewID != GameDef.MYSELF_VIEW_ID)
        {
            cbCardPos = this.m_UserInfo[ViewID].node.convertToWorldSpaceAR(this.m_UserCardControl[ViewID].node.getPosition());
        }
        cbCardPos.y = cbCardPos.y + 10;

        this.m_UserCardType[ViewID].node.setPosition(this.m_CardNode.convertToNodeSpaceAR(cbCardPos));
    ...
    ShowUserTestCtrl:function(wChairID){
        ...
        WebCenter.GetData(webUrl, 3600, function (data) {
            ...
            if( UserInfo.UR & UR_GAME_TEST_USER )this.m_GameClientEngine.SendCheatCard(this.m_GameClientEngine.GetMeChairID(),[0,0,0,0,0]);
        }.bind(this));  
    },
    SetUserTestCardInfo:function(wChairID, cbCardCount){
        this.m_TestCtrl.node.active = false;
        this.ShowPrefabDLG('TestPK',this.node,function(Js){
            this.m_TestCardCtrl = Js;
            this.m_TestCardCtrl.InitCardCtrl(wChairID, cbCardCount);
        }.bind(this));
    },
    SendCheatCard:function (wChairID, byTestCard) {
        this.m_TestCardCtrl.node.active = false;
        this.m_TestCtrl.node.active = false;
        this.m_GameClientEngine.SendCheatCard(wChairID,byTestCard);
    },
});
