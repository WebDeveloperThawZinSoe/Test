...
var CT_HULU        = 7;
var CT_TIAO_4      = 8;
var CT_TONGHUASHUN = 9;
var CT_TIAO_5      = 10;
var CT_TIAO_3      = 11;

var CGameLogic_501 = cc.Class({
    ...
    SearchSameCard: function (cbCardData, cbCardCnt, cbSameCnt) {
        ...
        while (cbKingCnt < cbCardCnt && this.IsKing(cbCard[cbKingCnt]))
            cbKingCnt++;
        if(cbKingCnt == cbSameCnt){
            var cbSameCard = new Array();
            cbSameCard[0] =cbCard[0];
            cbSameCard[1] =cbCard[1]; 
    ...
    SearchSameColorType: function (cbHandCardData, cbHandCardCount, cbSameCount) {
        ...
        for (var i = 0; i < cbHandCardCount; i++) {
            if (this.IsKing(cbCardData[i])) {
                ...
                if(cbCardData[i]==0x4E){
                    cbSameCardData[1].push(cbCardData[i]);
                    cbSameCardData[3].push(cbCardData[i]);
                }else if(cbCardData[i]==0x4F){
                    cbSameCardData[0].push(cbCardData[i]);
                    cbSameCardData[2].push(cbCardData[i]);
                }
                continue;
            }
        ...
        for (let i = 0; i < 4; i++) {
            if (cbSameCardData[i].length >= cbSameCount) {
                
                    let res = this._Choose(cbSameCardData[i], cbSameCount);
                    for (let j in res) {
                        let arr = res[j];
                        arr = arr.concat(0);
    ...
    GetCardType: function (cbCard) {
        ...
        if (result.cbSearchCount > 0) return CT_TIAO_3;
        
        result = this.SearchSameColorLineType(cbCard, cbCard.length, 3);
        if (result.cbSearchCount > 0) return CT_TONGHUASHUN;


        result = this.SearchSameColorType(cbCard, cbCard.length, 3);
        if (result.cbSearchCount > 0) return CT_TONGHUA;

        result = this.SearchLineCardType(cbCard, cbCard.length, 3);
    ...
    AnalysebCardData: function (cbCard, cbCardCount) {
        ...
        if (AnalyseResult.cbKingCnt == 1) {

            AnalyseResult.cbLogicValue[0].push(0);
        }
        if (AnalyseResult.cbKingCnt == 2) {

            AnalyseResult.cbLogicValue[1].push(16);
        }
...
CGameLogic_501._instance = null;

CGameLogic_501.getInstance = function () {
    if (!CGameLogic_501._instance) {
        CGameLogic_501._instance = new CGameLogic_501();
    }
    return CGameLogic_501._instance;
};
