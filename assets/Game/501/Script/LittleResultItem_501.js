cc.Class({
    ...
    _setScore: function (llScore) {
        ...
        if (llScore > 0) {
            this.m_labWinScore.string = '+' + llScore;
            this.m_labWinScore.node.active = true;
        } else if (llScore < 0) {
            this.m_labLoseScore.string = llScore;
    ...
    ShowItemView: function (cmd, wChairID, pGameView) {
        var pUserItem = pGameView.m_pChairUserItem[wChairID];
        ...
        this.$('@UserCtrl').SetUserByID(pUserItem.GetUserID());
