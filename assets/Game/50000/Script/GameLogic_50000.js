...
var CGameLogic = cc.Class({
    ...
    GetCardType:function ( cbCardData, cbCardCount)
    {
        ...
        if (AnalyseResult.cbBlockCount[2]>0)
        {
            ...
            //if (AnalyseResult.cbBlockCount[2]*3==cbCardCount) return (cbCardCount >= 6? GameDef.CT_AIRPLANE_ONE:GameDef.CT_THREE_LINE);
            //if (AnalyseResult.cbBlockCount[2]*4==cbCardCount) return(cbCardCount >= 8? GameDef.CT_AIRPLANE_ONE:GameDef.CT_THREE_TAKE_ONE);
            //if ((AnalyseResult.cbBlockCount[2]*5==cbCardCount)&&(AnalyseResult.cbBlockCount[1]==AnalyseResult.cbBlockCount[2]))
                //return (cbCardCount >= 10? GameDef.CT_AIRPLANE_ONE:GameDef.CT_THREE_TAKE_TWO);
    ...
    SearchThreeTwoLine:function ( cbHandCardData, cbHandCardCount, pSearchCardResult )
    {
        ...
        if( cbTmpResultCount > 0 )
        {
            ...
            for( var i = 0; i < cbTmpResultCount; i++ )
            {
                ...
                this.RemoveCard( tmpSearchResult.cbResultCard[i],tmpSearchResult.cbCardCount[i],
                        cbTmpCardData,cbTmpCardCount )
