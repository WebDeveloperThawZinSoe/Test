    ...
    PlaySendCard:function (cnt, start){
        for(var i=0;i<cnt;i++){
            for(var j=0;j<GameDef.GAME_PLAYER;j++){
                var wChairID = (j+start)%GameDef.GAME_PLAYER;
                //if(!this.m_Hook.m_GameClientEngine.m_cbUserState[wChairID]) continue;
                var TempCard = this.GetCard();
                this.SetNodeAct(TempCard.node, wChairID, i);
            }
        }
    },

    SetNodeAct:function (node, wChairID, index){
