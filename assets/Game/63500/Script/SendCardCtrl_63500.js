...
cc.Class({
    ...
    PlaySendCard:function (StateArr, wStartChairID, cbSendCount ){
        ...
        for(var j = 0; j < GameDef.GAME_PLAYER; ++ j) {
            ...
            for(var i = 0; i < cbSendCount; ++ i) {
                ...
                this.SetNodeAct(TempCard.node, j, i);
            }
        }
    },

    SetNodeAct:function (node, wChairID, cbCardData){
        this.m_ActCardCount++;
        ...
        {
            this.EndCallFunc(node, [wChairID, wViewID, cbCardData]);
            return;
        }
        node.runAction(cc.sequence(act, cc.callFunc(this.EndCallFunc.bind(this), this, [wChairID, wViewID, cbCardData])));
    },

    //node: TempCard.node  ; para = [wViewID, wChairID, cbCardCnt]
    EndCallFunc:function (node, para){
        cc.gSoundRes.PlaySound('SEND_CARD');
        this.DelCard(node);
        this.m_ActCardCount--;
        this.m_Hook.m_GameClientEngine.OnMessageDispatchFinish(para, this.m_ActCardCount);
    },
