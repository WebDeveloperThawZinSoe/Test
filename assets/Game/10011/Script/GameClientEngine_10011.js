...
cc.Class({
    ...
    ctor: function () {
        ...
        //牌型声音
        for (var i = 2; i <= 6; ++i) {
            this.m_SoundArr[this.m_SoundArr.length] = ['M_' + (100 + i), 'm/type/' + (100 + i)];
            this.m_SoundArr[this.m_SoundArr.length] = ['W_' + (100 + i), 'w/type/' + (100 + i)];
        }
                    ...
                    for (var i = 0; i < GameDef.GAME_PLAYER; i++) {
                        ...
                        if (ViewID == GameDef.MYSELF_VIEW_ID) {
                            ...
                            this.m_GameClientView.ShowOpenUI(
                                ...
                                ,GameDef.GetCardCount(this.m_dwServerRules, this.m_dwRulesArr) == GameDef.MAX_COUNT);
                        }

                        //玩家手牌
                        this.m_GameClientView.m_UserCardControl[ViewID].SetCardData(pStatusPlay.cbHandCardData[i], pStatusPlay.iSendCardCount);
                        this.m_GameClientView.m_UserCardControl[ViewID].SetBlackCard(pStatusPlay.cbRemoveCardIndex[i]);

                        this.m_GameClientView.SetCardType(ViewID, pStatusPlay.cbOxType[i],pStatusPlay.cbJHType[i]);

                        if (ViewID != GameDef.MYSELF_VIEW_ID)
                            this.m_GameClientView.ShowCuoPaiState(ViewID, pStatusPlay.cbPlayStatus[i] == GameDef.PLAYER_STATUS_CUO_PAI);
                    }
                    return true;
                }
        }
    ...
    OnSubStartOpen: function (pData, wDataSize) {
        ...
        this.m_GameClientView.ShowOpenUI(false, true,GameDef.GetCardCount(this.m_dwServerRules, this.m_dwRulesArr) == GameDef.MAX_COUNT);
    ...
    OnSubHintCard: function (pData, wDataSize) {
        var HintCard = GameDef.CMD_S_Hint_Card();
        //效验
        if (wDataSize != gCByte.Bytes2Str(HintCard, pData)) return false;

        //显示牌
        this.m_GameClientView.SetCardType(GameDef.MYSELF_VIEW_ID, HintCard.cbOxType, HintCard.cbOxTypeJH);


        var MeChair = this.GetMeChairID();
        var bGame = this.m_UserState[MeChair];
        if (!this.IsLookonMode())
            this.m_GameClientView.ShowOpenUI(bGame, false,GameDef.GetCardCount(this.m_dwServerRules, this.m_dwRulesArr) == GameDef.MAX_COUNT);
    ...
    OnSubOpenCard: function (pData, wDataSize) {
        ...
        this.m_GameClientView.m_UserCardControl[ViewID].SetCardData(OpenCard.cbCardData, GameDef.GetCardCount(this.m_dwServerRules, this.m_dwRulesArr));
        this.m_GameClientView.m_UserCardControl[ViewID].SetBlackCard(OpenCard.cbRemoveCardIndex);
        this.m_GameClientView.SetCardType(ViewID, OpenCard.cbOxType, OpenCard.cbOxTypeJH);

        var sequence = cc.sequence(
            cc.callFunc(function()
            {
                if (OpenCard.cbOxType >= 1 && OpenCard.cbOxType <= 14)
                    this.PlayActionSound(OpenCard.wChairID, OpenCard.cbOxType)
            }, this),
            cc.delayTime(0.5), 
            cc.callFunc(function()
            {
                if (OpenCard.cbOxTypeJH >= 2 && OpenCard.cbOxTypeJH <= 6)
                    this.PlayActionSound(OpenCard.wChairID, (100 + OpenCard.cbOxTypeJH))
            }, this)
        );
    ...
    OnSubLookCard: function (pData, wDataSize) {
        ...
        this.m_GameClientView.m_UserCardControl[ViewID].SetCardData(lookCard.cbCardData, GameDef.GetCardCount(this.m_dwServerRules, this.m_dwRulesArr));
        
        //开启触摸
        // if (GameDef.GetCardCount(this.m_dwServerRules, this.m_dwRulesArr) == GameDef.MAX_COUNT &&ViewID == GameDef.MYSELF_VIEW_ID)
        //     this.m_GameClientView.m_UserCardControl[ViewID].SetGameEngine(this);

        var MeChair = this.GetMeChairID();
        var bGame = this.m_UserState[MeChair];

        var bIsMaxCount =  GameDef.GetCardCount(this.m_dwServerRules, this.m_dwRulesArr) == GameDef.MAX_COUNT;

        this.m_GameClientView.ShowLookUI(false);
        this.m_GameClientView.ShowOpenUI(bGame && !this.IsLookonMode(),true, bIsMaxCount);
    ...
    OnSubCtrlOpenCard: function (pData, wDataSize) {
        ...
        if (CtrlOpenCard.wChairID == MeChair && !this.IsLookonMode()) {
            ...
            this.ShowPrefabDLG('NdOpenCard2',null,function(Js){
                Js.SetCardData(CtrlOpenCard.cbCardData[GameDef.GetCardCount(this.m_dwServerRules, this.m_dwRulesArr) - 1]);
            ...
            this.m_GameClientView.ShowLookUI(false);
            this.m_GameClientView.ShowOpenUI(bGame && !this.IsLookonMode(),true, GameDef.GetCardCount(this.m_dwServerRules, this.m_dwRulesArr) == GameDef.MAX_COUNT);
        }
    ...
    OnSubOpenResult: function (pData, wDataSize) {
        ...
        for (var i = 0; i < GameDef.GAME_PLAYER; i++) {
            ...
            //玩家手牌
            this.m_GameClientView.m_UserCardControl[ViewID].SetCardData(pResult.cbCardData[i], GameDef.GetCardCount(this.m_dwServerRules, this.m_dwRulesArr));
            this.m_GameClientView.m_UserCardControl[ViewID].SetBlackCard(pResult.cbRemoveCardIndex[i]);

            this.m_GameClientView.SetCardType(ViewID, pResult.cbOxType[i],pResult.cbJHType[i]);
        }
    ...
    OnSubGameConclude: function (pData, wDataSize) {
        ...
        for (var i = 0; i < GameDef.GAME_PLAYER; i++) {
            ...
            this.m_GameClientView.m_UserCardControl[ViewID].SetCardData(this.m_GameEnd.cbCardData[i], GameDef.GetCardCount(this.m_dwServerRules, this.m_dwRulesArr));
            this.m_GameClientView.m_UserCardControl[ViewID].SetBlackCard(this.m_GameEnd.cbRemoveCardIndex[i]);

            this.m_GameClientView.SetCardType(ViewID, this.m_GameEnd.cbOxType[i], this.m_GameEnd.cbJHType[i]);
    ...
    //提示
    OnMessageHint: function (TouchIndex) {
        //设置界面
        this._UserChooseIndex = TouchIndex;

        var Combination = GameDef.CMD_C_CombinationCard();
        Combination.cbRemoveData = TouchIndex;
        //发送数据
        this.SendGameData(GameDef.SUB_C_HINT,Combination);
    },
    //组牌
    OnMessageCombiCard: function () {
        //设置界面
        this.m_GameClientView.HideAllGameButton();

        if (this._UserChooseIndex == null || this._UserChooseIndex == undefined) return;

        var Combination = GameDef.CMD_C_CombinationCard();
        Combination.cbRemoveData = this._UserChooseIndex;
        //发送数据
        this.SendGameData(GameDef.SUB_C_OPEN_CARD,Combination);
    },
    //亮牌
    OnMessageOpenCard: function () {
        ...
        var Combination = GameDef.CMD_C_CombinationCard();
        Combination.cbRemoveData = GameDef.MAX_COUNT;
        //发送数据
        this.SendGameData(GameDef.SUB_C_OPEN_CARD,Combination);
    },
    ...
    SetViewRoomInfo: function (dwServerRules, dwRulesArr) {
        ...
        GameDef.SetRules(dwRulesArr,dwServerRules);
