cc.Class({
    ...
    Init:function(){
        ...
        this.m_BaseItemNd.active = false;

        var Toggle = this.CopyItem(this.m_BaseItemNd,this.m_ItemNd).getComponent(cc.Toggle);
        Toggle.node.name = `${window.SetKey_Card_Back}_${0xff}`;
        var cstr = '255';
        this.$('Background@Sprite',Toggle.node).spriteFrame = this.m_Atlas.getSpriteFrame(cstr);
    ...
    Load: function () {
        this.scheduleOnce(this.OnTimer_IDI_Load, 0.01);
    },

    OnTimer_IDI_Load: function () {
        if (!this.m_ToggleArr) {
            this.m_ToggleArr = [];
