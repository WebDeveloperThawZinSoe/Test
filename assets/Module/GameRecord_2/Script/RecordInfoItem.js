cc.Class({
    ...
    InitPre:function(){
        ...
        this.m_ListCtrl.InitList(0, 'RecordUserItem2', this);
        this.node.active = false;
    },
    SetPreInfo:function(ParaArr){//0 Kind  1 RecordID 2 index 3 info
        this.node.active = true;
        this.m_KindID = ParaArr[0];
        this.m_RecordID = ParaArr[1];
        this.m_GameIndex = ParaArr[2];
        this.m_ScoreInfo = ParaArr[3];

        this.$('Info/LabTime@Label').string = this.m_ScoreInfo['Time'];
        this.$('Info/LbIndex@Label').string = pad(this.m_GameIndex, 2) ;

        var sortIDArr = this.SortUserIndexByScore(this.m_ScoreInfo);
        for (var i in sortIDArr)
        {
            this.m_ListCtrl.InsertListInfo(0, [sortIDArr[i], this.m_ScoreInfo[sortIDArr[i]]]);
        }
